...
public class UserServiceImpl implements UserService {
	...
	public Boolean sendOtp(String email) throws Exception {
	    ...
	    System.out.println("Looking for user with email: " + normalizedEmail);
   ...
   User user=useRepository.findByEmail(loginDTO.getEmail()).orElseThrow(()-> 
   new JobPortalException("USER_NOT_FOUND"));
   user.setPassword(passwordEncoder.encode(loginDTO.getPassword()));
   userRepository.save(user);	   
   NotificationDTO noti=new NotificationDTO();
   noti.setUserId(user.getId());
   noti.setMessage("Password reset succesfully");
   ...
   return new ResponseDTO("Password changed  succesfully ");
    ...
    if(!expiredOTPs.isEmpty())
    {
    	...
    	System.out.print("Removed"+expiredOTPs.size()+"expired OTP");
      }
	}
