...
const MultiInput = (props: MultiInputProps) => {
  const [data, setData] = useState<string[]>(Array.isArray(props.options) ? props.options : []);
  ...
  const handleValueSelect = (val: string) => {
    ...
    if (val === '$create') {
      ...
      dispatch(updateFilter({ [props.title as string ] : [...value, search] })) ;
    } else {
   dispatch(updateFilter({ 
   [props.title as string]: value.includes(val) ? value.filter((v) => v !== val) : [...value, val] 
   }));
      setValue((current) =>
        current.includes(val) ? current.filter((v) => v !== val) : 
      [...current, val])
     }
  };

  const handleValueRemove = (val: string) =>{
    dispatch(updateFilter({[props.title as string ]:value.filter((v)=>v !== val )}));
    ...
    <Combobox store={combobox} onOptionSubmit={handleValueSelect} withinPortal={false}>
      <Combobox.DropdownTarget>
        <PillsInput variant='unstyled' rightSection={<IconSelector />} onClick={() =>
          ...
          leftSection={
            <div className='text-amber-400 p-1 bg-mine-shaft-900 rounded-full mr-2'><props.icon/></div>
          }
        >
          <Pill.Group>
            {value.length > 0 ? (
