...
return <div  className="p-4 rounded-xl bg-mine-shaft-900 hover:shadow-[0_0_5px_1px_yellow] !shadow-bright-sun-400 transotion
duration-300 ease-in-out w-72 flex flex-col gap-3">
    <div className="flex justify-between">
        ...
        <div className="flex flex-col gap-1">
          ...
          className="hover:text-mine-shaft-200" to="/company" >{props.company}</Link>  &bull ;
  ...
  { profile.savedJobs?.includes(props.id)?<IconBookmarkFilled 
   ...
   stroke={1.5}/>: <IconBookmark onClick={handleSaveJob}  className="cursor-pointer text-mine-shaft-300
   hover:text-amber-400" stroke={1.5}/> }
</div>
<div>
    <div className="p-2 py-1 bg-mine-shaft-800 text-bright-sun-400 rounded-lg text-xs ">{props.experiance}</div>
...
<div>
    <Text className="!text-xs text-justify !text-mine-shaft-300" lineClamp={3}>{props.about } 
     ...
     <Divider color="mineShaft.7" size="xs" /><div className="flex justify-between">
    ...
    props.offered &&<div className="flex gap-2">
      <Button color="brightSun.4" variant="outline" fullWidth>Accepted</Button>
    ...
    <IconCalendarMonth className="text-amber-400 w-5 h-5" stroke={1.5}/>Sun, 25 August &bull; <span className="text-mine-shaft-400" >10:00 AM</span>10:00 Am 
    </div>
    }
