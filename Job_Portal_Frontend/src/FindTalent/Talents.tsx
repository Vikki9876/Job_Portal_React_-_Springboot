...
const Talents=()=>{
      ...
      useEffect(()=>{
        ...
        if(filter["Job Title"] && filter["Job Title"].length>0)
          ...
          if(filter.Skills && filter.Skills.length>0 )
            {
            filterTalent=filterTalent.filter((talent:any)=>filter.Skills?.some((skill:any)=>talent.skills?.some((talentSkill:any)=>talentSkill.toLowerCase().includes()))) ;    
            }
            if(filter.exp && filter.exp.length>0 )
            {
            filterTalent=filterTalent.filter((talent:any)=>filter.exp[0]<=talent.totalExp &&
  ...
  <div className="flex mt-10 flex-wrap gap-5 justify-start">
    {
        filteredTalents?.length?filteredTalents.map((talent:any,index:any )=> 
        <TalentCard key={index} {...talent}/> ):<div className="text-xl font-semibold">
            No Talents found </div>
          }  
         </div>
    </div>
}
